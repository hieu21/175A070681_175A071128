...
<?php
	...
	if (isset($_POST["doimatkhaugv"])) {
	    ...
	    $update_pwd = mysqli_query($conn, 
			...
			echo "<script>alert('Đổi mật khẩu thành công!'); window.location='giangvien.php'</script>";
		}
		else{
			echo "<script>alert('Mật khẩu với và mật khẩu mới nhập lại không trùng nhau! Yêu cầu nhập lại'); window.location='giangvien.php'</script>";
		}
	}
	else
	{
		echo "<script>alert('Mật khẩu cũ của bạn sai!'); window.location='giangvien.php'</script>";
	}
...
<body>
    ...
    <main>
            <div class="wrapper">
                <div class="login">
				<form action="doimatkhaugv.php" method="POST">	
					...
					<div class="form-group" >
							<input type="submit" name="doimatkhaugv"  value="Đổi mật khẩu" class="btn btn-success" style="margin-top: 25px; margin-left: 0px;"/>
					</div>
					<?php echo $message; ?>                   
					</form>
                </div>
